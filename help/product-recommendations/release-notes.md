...
![Fehler](../assets/bug.svg) Produkte, die aus einem [!DNL Product Recommendations] -Einheit auf der Warenkorbseite nicht aus der Liste der empfohlenen Produkte entfernt, wenn die Warenkorbseite neu geladen wird.
![Fehler](../assets/bug.svg) Aus dem Warenkorb entfernte Produkte bleiben weiterhin im `cartSkus` -Array, bis die Warenkorbseite neu geladen wird.
...
![Fehlerbehebung](../assets/fix.svg) Es wurde ein Problem behoben, bei dem das Klicken auf **[!DNL Add to Cart]** -Schaltfläche in einem einfachen Produkt innerhalb einer Produkteinheit in Recommendations wurde der Käufer auf die Startseite umgeleitet, anstatt auf der aktuellen Seite zu bleiben.
![Fehler](../assets/bug.svg) Es gibt einen Validierungsfehler, der durch die `referenceBlock` -Element im `ProductRecommendations Layout` XML-Datei.
